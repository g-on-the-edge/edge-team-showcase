    ...
    <nav className="fixed top-0 left-0 right-0 z-[100] bg-black/80 border-b border-white/10 backdrop-blur-2xl">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between text-white">
        <div className="flex items-center">
          <img 
            src="/Gundersen_Edge_RGB_Red_large_tagline.png" 
            alt="GHS Edge - On the Other Side of Possible" 
            className="h-12 w-auto"
          />
        </div>
        
        <div className="flex gap-6 text-sm tracking-[0.3em] uppercase">
          {[
            { label: 'Home', id: 0, href: '#section-0' },
            { label: 'Showcase', id: 1, href: '#section-1' },
            { label: 'Portfolio', id: 2, href: '#section-2' },
            { label: 'Highlights', id: 3, href: '#section-3' },
          ].map((item) => (
            <a
              key={item.label}
              href={item.href}
